<table id="tabel-data" class="table table-bordered table-bordered dt-responsive nowrap">
    ...
    <tbody>
        <?php
        include "../../config.php";
        $query = mysqli_query($conn, "SELECT * FROM v_returnpenjualan");
        while ($data = mysqli_fetch_array($query)) {
            ...
            <tr>
                ...
                <td><?= $data['catatan'] ?></td>
                <td>
                    <button data-id="<?= $data['id_return_penjualan'] ?>" id="edit" type="button"
                        class="btn btn-primary">Edit</button>
                    <button data-id="<?= $data['id_return_penjualan'] ?>" id="delete" type="button"
...
<script>
    $(document).ready(function () {
        ...
        $('#tabel-data').on('click', '#edit', function () {
            ...
            $.ajax({
                type: 'POST',
                url: 'page/return-penjualan/edit-return-penjualan.php',
                data: 'id=' + id,
        ...
        $('#tabel-data').on('click', '#delete', function () {
            ...
            alertify.confirm('Hapus', 'Apakah anda yakin ingin menghapus transaksi ini? ', function () {
                $.ajax({
                    type: 'POST',
                    url: 'proses.php?act=hapus-return-penjualan',
                    data: {
                        id: id,
