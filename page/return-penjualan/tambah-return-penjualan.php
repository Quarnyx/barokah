<form id="tambah-return-penjualan" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        $query = mysqli_query($conn, "SELECT MAX(kode_return_penjualan) AS kode_return_penjualan FROM return_penjualan");
        $data = mysqli_fetch_array($query);
        $max = $data['kode_return_penjualan'] ? substr($data['kode_return_penjualan'], 3, 3) : "000";
        $no = $max + 1;
        $char = "RPJ";
...
<script>
    $("#tambah-return-penjualan").submit(function (e) {
        e.preventDefault();
        var formData = new FormData(this);
        $.ajax({
            type: 'POST',
            url: 'proses.php?act=tambah-return-penjualan',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
